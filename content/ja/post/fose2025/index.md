...
白川さんは「継続的インテグレーションにおけるテスト連続失敗（Test alert snooze）の実証分析」を発表しました．既存研究では，継続的インテグレーション（CI）におけるテスト連続失敗について実態や原因などが明らかになっていません．そこで，本研究では，テストメゾットが連続した失敗をTest alert snoozeと定義し，その特徴を実証分析しました．その結果，Test alert snoozeの割合，継続して失敗した数，修正時間が，棒グラフと箱ひげ図で確認できました．
